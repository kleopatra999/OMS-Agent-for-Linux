...
module Tailscript
  ...
  class NewTail  
    ...
    def setup_watcher(path, pe)
      tw = TailWatcher.new(path, pe, @read_from_head, &method(:receive_lines))
      tw.on_notify
      tw
    end
    ...
    class TailWatcher
      def initialize(path, pe, read_from_head, &receive_lines)
        @path = path
      ...
      def on_rotate(io)
        if io
          ...
          io.seek(pos)
          @io_handler = IOHandler.new(io, @pe, &method(:wrap_receive_lines))
        else
